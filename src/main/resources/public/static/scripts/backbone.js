...
define('backbone', function(require) {
  ...
  _.extend(Backbone.History.prototype, Backbone.Events, {
    ...
    // you which to modify the current URL without adding an entry to the
    // history.
    navigate : function(fragment, options) {
      if (!historyStarted)
        return false;
      ...
      if (this._hasPushState) {
        ...
        if (this.iframe && (frag != this.getFragment(this.iframe.location.hash))) {
          ...
          // history entry on hash-tag change.
  ...
  Backbone.sync = function(method, model, options) {
    ...
    if (method === 'read' && supports_local_storage()) {
      ...
      var url = params.url || options.url;
      var namespace = model.cache_namespace || "_", key = "backbone-cache/" + namespace + "/" + url, val = localStorage.getItem(key), successFn = options.success;
